...
class IntersectionOverUnion(Metric):
    ...
    def update(self, prediction: torch.Tensor, target: torch.Tensor):
        tps, fps, _, fns, sups = stat_scores(preds=prediction, target=target, num_classes=self.n_classes, task='multiclass')
    ...
    def compute(self):
        """
        Compute the Intersection Over Union (IoU) scores for each class.

        Returns:
            torch.Tensor: A tensor containing the IoU scores for each class. The shape of the tensor is
            (n_classes - 1,), where n_classes is the number of classes excluding the ignored class.

        Notes:
            - The IoU score is computed as the ratio of true positives (tp) to the sum of true positives,
              false positives (fp), and false negatives (fn) for each class.
            - If a class is absent in both the target and the prediction, the IoU score for that class is
...
class PanopticMetric(Metric):
    def __init__(
        ...
        Parameters:
            ...
            temporally_consistent (bool, optional): Whether the instance segmentation is temporally consistent. Defaults to True.
            vehicles_id (int, optional): The ID of the vehicles class. Defaults to 1.
            compute_on_step (bool, optional): Whether to compute the metric on each step. Defaults to False.

        Initializes the following attributes:
            - n_classes (int): The number of classes.
            - temporally_consistent (bool): Whether the instance segmentation is temporally consistent.
            - vehicles_id (int): The ID of the vehicles class.
            - keys (List[str]): The keys for the metrics.
            - iou (torch.Tensor): The tensor for storing the IoU values.
            - true_positive (torch.Tensor): The tensor for storing the true positive values.
            - false_positive (torch.Tensor): The tensor for storing the false positive values.
            - false_negative (torch.Tensor): The tensor for storing the false negative values.
