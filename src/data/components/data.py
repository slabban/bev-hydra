...
class FuturePredictionDataset(torch.utils.data.Dataset):
    def __init__(self, is_train : bool, data_root : str, version : str, batch_size : int, filter_invisible_vehicles : bool, common: DictConfig = None): 
        """
        Constructor for the FuturePredictionDataset clas based on the NuScenes dataset.

        Args:
        - is_train: Whether we are in training or validation mode.
        - data_root: The root directory of the dataset.
        - version: The version of the dataset.
        - batch_size: The batch size.
        - filter_invisible_vehicles: Whether to filter out vehicles which are not visible.
        - common: A dictionary containing common parameters, such as the receptive field.

        Extracts: 
        - The indices of the scenes in the dataset and chunk them into valid sequences.
        - The image augmentation parameters -> resizing and cropping parameters.
        - The the bird's-eye view parameters, needed for for BEV labaels.
        """
    ...
    def get_input_data(self, rec):
        ...
        -------
            ...
            intrinsics: torch.Tensor<float> (N, 3, 3)
        ...
        # The extrinsics we want are from the camera sensor to "flat egopose" as
        # defined in https://github.com/nutonomy/nuscenes-devkit/blob/9b492f76df22943daf1dc991358d3d606314af27/python-sdk/nuscenes/nuscenes.py#L279
        # which corresponds to the position of the lidar.
        # This is because the labels are generated by projecting the 3D bounding
        # box in this lidar's reference frame.
    ...
    def get_birds_eye_view_label(self, record, instance_map):
        translation, rotation = self._get_top_lidar_pose(record)
    ...
    def _get_poly_region_in_image(self, instance_annotation, ego_translation, ego_rotation):
        """
        Given the instance annotation, and the ego pose (transformation from world to ego-car coordinates)
        in the form of translation and rotation, compute the 2D projection of the bounding box
        in the bird's-eye view (BEV) image and the z-coordinate of the bottom of the bounding box.

        The computation is done by transforming the box from world coordinates to ego-car coordinates,
        and then computing the BEV projection of the bottom corners of the box.
        """
        box = Box(
            instance_annotation['translation'], instance_annotation['size'], Quaternion(instance_annotation['rotation'])
        )

        # First, translate the box to ego-car coordinates
        box.translate(ego_translation)

        # Then, rotate the box to ego-car coordinates
        box.rotate(ego_rotation)

        # Compute the 2D projection of the bottom corners of the box
        pts = box.bottom_corners()[:2].T

        # The BEV image is defined so that the x-axis points forward, the y-axis points to the right,
        # and the origin is at the bottom left corner of the image.
        # So, we need to shift the points so that the origin is at the bottom left corner of the BEV image.
        # Also, we need to flip the y-axis to match the NuScenes coordinate system.
        pts = np.round((pts - self.bev_start_position[:2] + self.bev_resolution[:2] / 2.0) / self.bev_resolution[:2]).astype(np.int32)
        pts[:, [1, 0]] = pts[:, [0, 1]]

        # Compute the z-coordinate of the bottom of the bounding box
    ...
    def get_label(self, rec, instance_map):
        """
        Gets the segmentation, instance, z-position, instance_map, and attribute labels for
        a given record.

        Parameters
        ----------
        rec: dict
            Record from NuScenes dataset.
        instance_map: dict
            Map from instance token to instance id.

        Returns
        -------
        segmentation: torch.Tensor
            Segmentation mask, shape (1, 1, height, width).
        instance: torch.Tensor
            Instance mask, shape (1, height, width).
        z_position: torch.Tensor
            Height of each pixel in the bird's eye view, shape (1, 1, height, width).
        instance_map: dict
            Updated instance map.
        attribute_label: torch.Tensor
            Attribute label, shape (1, 1, height, width).
        """
